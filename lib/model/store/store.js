...
const getDealsfromStoresArray = (filteredStores) => {
    ...
    filteredStores.forEach((store) => {
        store.deals.forEach((deal) => {
            ...
            deal = { ...deal, name: store.name, distance: store.distance };
...
const editStore = async (id, store) => {
    ...
    if (store.images !== 'undefined' && store.images != null && Array.isArray(store.images) && store.images.length > 0) {
        store.photos = await uploadBulkPhoto(store.images, userID, 'store');
        delete store.images;
    }
...
const getLastTizers = async() => {
    res = await storeModel.find({tizers: {$exists: true, $ne: []}}).select('tizers -_id');
    var arr = [];
