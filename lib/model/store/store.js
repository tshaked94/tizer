...
async function findByCategory(category, distance, userCoordinates) {
    ...
    filteredStores.forEach(async (store) =>{
        ...
        // const filteredDeals = await store.deals.filter(async function(deal) {
        //     console.log(deal.categories.includes(category)? deal + ' contains ' + category : deal + ' not contains ' + category);
        //     return deal.categories.includes(category);
        // })
        // store.deals = filteredDeals;
    });
    // const result = await filteredStores.filter(async function(store) {
    //     const filteredDeals = await store.deals.filter(async function(deal) {
    //         return deal.categories.includes(category);
    //     });
    //     store.deals = filteredDeals;
