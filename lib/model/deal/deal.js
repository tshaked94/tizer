...
const editDeal = async (id, deal) => {
    ...
    if (deal.photos == 'undefined' || deal.photos == null || deal.photos == []) {
        ...
        deal.photos.forEach((photo, index) => {
            photoObj = photo.toObject();
            photoObj.createdAt = new Date(photoObj.createdAt).getTime();
            photoObj.updatedAt = new Date(photoObj.updatedAt).getTime();
            deal.photos[index] = photoObj;
        });
    }

    console.log('after: ' + deal.photos);
